...
(async () => {
  ...
  /* ★ 2026-09-16 대표님 지시 "맨 밑에 한 줄만 있으면 되지 않니" — 상품마다 붙이던 것을
     목록 맨 아래 한 덩이로 모았습니다. 다만 단품과 이용권은 기간이 다르므로 **두 줄**입니다.
     한 줄로 뭉치면 둘 중 하나가 거짓이 됩니다. */
  const provides = (o.text.match(/서비스 제공기간/g) || []).length;
  R.note(provides === 1, '서비스 제공기간이 한 덩이로만 적혀 있다', provides + '개');
  R.note(/기한 없이 다시 볼 수 있어요/.test(o.text), '단품은 기한이 없다고 적었다');
  R.note(/12월 31일/.test(o.text) && /최대 1년/.test(o.text), '이용권 기간이 적혀 있다');
  R.note(!/고르신|결제하신|보실 수/.test(o.text),
         '해요체에 과한 존대를 섞지 않았다 (2026-09-16 대표님 지시)');
