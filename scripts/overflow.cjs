...
/* ★ 이미 알고 있는 것 — 이번 변경 때문이 아닌 자리다. 새 넘침만 걸리게 하려고 둔다.
   ★ 여기에 줄을 늘릴 때는 **왜 괜찮은지**를 반드시 함께 적으십시오. 이유 없이 늘리면
     이 검사는 그냥 꺼진 검사가 됩니다. */
const KNOWN = [
  /* ★ 2026-09-09 — 이유를 실제 원인으로 고쳐 적었습니다. 예전에는 '예전부터 있던 것'이라고만
     적혀 있었는데, 그건 **왜 괜찮은지가 아니라 언제부터인지**입니다. 그렇게 적으면
     고쳐야 할 것과 안 고쳐도 되는 것이 구별되지 않습니다.
     이 2px 은 오행 배지를 캐릭터 오른쪽 아래 모서리에 **일부러 걸쳐 놓은 것**입니다
     (`.hd-elem-badge{right:-2px}` · 시안 규칙). 감싼 칸에 overflow:hidden 이 없어서
     **잘리지 않고 그대로 보입니다** — 글자가 잘리는 종류가 아닙니다. */
  {sel:'hd-figure', why:'오행 배지를 일부러 2px 걸쳐 놓은 자리. 잘리지 않고 그대로 보인다', max:4},
  /* ★ char-dex-item(도감 칸) 은 2026-09-09 에 **뺐습니다.** 320·360·390·1280 네 폭에서
     다시 재 보니 넘침이 0건이었습니다. 안 넘치는 자리를 면제 목록에 두면 나중에 진짜로
     넘칠 때 그것을 조용히 통과시킵니다 — 면제는 확인될 때만 남깁니다. */
];
