...
```
fortune-server/
├── fortune.html          ← 서비스 (output/fortune 에서 복사해 옵니다)
...
- [ ] 테스트 카드로 결제 → 서비스로 돌아오며 **유료 섹션이 열리는가**
...
### 5단계 — 마지막에 서비스의 서버 주소를 켠다

**이 단계를 먼저 하면 안 됩니다.** 주소를 넣는 순간 서비스가 "서버 권위 모드"로 바뀌는데,
...
이건 서비스가 이미 알고 감수하기로 한 사항이고, 그 이유가 `fortune.html` 상단 주석에 적혀 있습니다.
...
| 서버 장애 시 무료 개방 | 실패하면 500을 반환 → 서비스가 잠금으로 처리 (열어주는 쪽으로 실패하지 않음) |
