...
export default async function handler(req, res) {
  ...
  try {
    ...
    /* ★ 2026-08-22 — 이 세션이 운영자(테스트 허가를 받은 사람)인지 알려준다.
       화면은 이 값이 참일 때만 [문의 관리] 입구를 만든다.
       ★ 이 값으로 유료 잠금을 열지 않는다 — 그건 위에서 ent.pass가 이미 했다.
         여기 있는 것은 '운영자 화면을 보여줄까'뿐이고, 실제 권한 판정은 다시 서버가 한다.
