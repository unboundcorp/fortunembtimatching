/* =====================================================================
   AI 해석의 공통 부분 — 프롬프트·캐시 열쇠·권한
   ---------------------------------------------------------------------
   ★ 두 곳에서 같은 AI를 부른다.
       api/interpret.js — 써지는 대로 흘려보낸다(스트리밍). 화면에 바로 얹힌다.
       api/content.js   — 다 만들어 놓고 한 번에 준다. 유료 본문 조회 규격(contentEndpoint).
     프롬프트를 양쪽에 복사해 두면 언젠가 둘이 달라지고, 그때는 같은 상품인데
     결과가 다른 이유를 아무도 못 찾는다. 그래서 여기 한 곳에만 둔다.

   ★ 이 파일은 열쇠를 직접 쓰지 않는다. 부르는 쪽에서 process.env로 꺼내 넘긴다.
===================================================================== */
import crypto from 'node:crypto';
import { testAccessOf, paidOrdersOf, aiAlreadyUsed, getAiCache, latestAiForSubject } from './store.js';
import { splitProductId, aiQuotaOf } from './products.js';
import { buildEntitlements } from './entitlements.js';

export const MODEL = 'claude-sonnet-5';
/* ★ 2026-08-25 — 12000 → 20000. 대운 세 섹션이 실제 내용으로 채워지면서 글이 길어졌고,
   그 바람에 상한에 걸려 마지막 섹션(클로징)이 통째로 잘린 채 저장됐다(실측: 5,311자에서
   "무리하게 " 로 끊김). 한글은 같은 길이라도 영어보다 토큰을 두세 배 먹는다 —
   글자 수만 보고 "아직 여유 있다"고 판단하면 안 된다.
   ★ 상한을 올리는 것만으로는 부족하다. 잘렸는지 확인하는 검사가 함께 있어야 한다
     (api/interpret.js의 섹션 개수·stop_reason 검사). 둘 중 하나만 있으면 언젠가 또 잘린다. */
export const MAX_TOKENS = 20000;
export const ANTHROPIC_URL = 'https://api.anthropic.com/v1/messages';

/* AI가 쓸 수 있는 상품만. 없는 상품 이름을 보내 요금을 쓰게 만들 수 없다.
   ★ 열쇠는 연도가 붙지 않은 이름이다. 'saju_full:2027' 같은 것은 aiKindOf()가 갈라서 본다. */
export const AI_PRODUCTS = { saju_full: 'saju', mbti_full: 'mbti', compat_full: 'compat' };

/* 'saju_full:2027' 처럼 연도가 붙어 와도 무슨 갈래인지 알아낸다.
   ★ AI_PRODUCTS[productId] 로 바로 찾으면 연도가 붙은 순간 undefined가 되어
     "요청 내용이 올바르지 않아요"로 막힌다. 두 창구가 이 함수 하나를 쓴다. */
export function aiKindOf(productId) {
  return AI_PRODUCTS[splitProductId(productId).base] || null;
}

/* ★ 프롬프트를 고치면 이 숫자를 올린다.
   캐시 열쇠는 payload에서만 뽑기 때문에, 시스템 프롬프트만 바꾸면 열쇠가 그대로다.
   그러면 말투를 존대로 바꿔도 예전에 만들어 둔 반말 글이 캐시에서 그대로 나온다.
   (실제로 겪었다 — 지식 문서에는 '해요체'라고 적어 두고 프롬프트에는 '반말체'가 남아 있어
    AI가 반말로 썼고, 그걸 고친 뒤에도 옛 글이 나올 뻔했다.)
   2 = 말투를 해요체로 통일한 판.
   3 = 궁합(compat_full)을 AI가 쓰기 시작한 판. 이름 자리표시자 규칙을 넣었다.
   5 = 대운·세운을 payload에 넣은 판 (2026-08-25). 그 전에 만들어 둔 글에는 대운 세 섹션이
       비어 있거나 잘려 있으므로, 열쇠를 바꿔 다시 만들게 한다. 지우지 않고 비켜 간다.
   7 = 해요체를 못 벗어나게 못 박은 판 + 표의 숫자에 모수·합계를 적게 한 판
       (둘 다 2026-08-29 대표님 지시). 표 쪽은 "이 개수는 뭔데?"라는 물음에서 나왔다 —
       결의 개수를 넘겨주면서 그게 무엇을 센 숫자인지는 안 알려 주고 있었다.
       그전 지침은 "반말을 섞지 마세요"까지만 적혀 있어서, AI가 합니다체로 새는 것을
       못 막았다. 합니다체도 존댓말이라 규칙을 어긴 줄 모른 것이다.
       실제로 6,079자 풀이에 "~습니다"로 끝난 문장이 세 개 섞여 있었다.
       ★ 이 숫자를 안 올리면 그 섞인 글이 캐시에서 그대로 다시 나온다.
   9 = 사주 용어를 문장에 그대로 쓰지 못하게 못 박은 판 (2026-08-31 대표님 지시).
       "오늘 날짜의 아랫글자가 타고난 여덟 글자와 정면으로 부딪혀요" 같은 문장이 나가고 있었다.
       읽는 사람은 ㉠ 아랫글자가 뭔지 ㉡ 여덟 글자 중 무엇과 부딪히는지 ㉢ 그래서 어떻다는 건지를
       하나도 알 수 없다. 대표님 말씀 그대로 옮기면 —
       "오늘 날짜의 어떤 사주와 내 사주가 어떤 점에서 부딪혀요! 이런 식으로 풀어서 설명해줘야
        사람들이 알아먹는다".
       ★ 이 숫자를 안 올리면 예전에 만들어 둔 '용어 그대로' 글이 캐시에서 그대로 다시 나온다.
  12 = 말을 더 쉽게 — 8-2~8-5번을 넣은 판 (2026-09-01 대표님 지시: "너 말이 계속 어려워").
       본문 한자 금지 · 용어는 처음 한 번만 쓰고 그 자리에서 풀기 · 한 글에 용어 6개까지 ·
       비유는 하나로 · 되돌리기 어려운 결정에는 보기 들기 · 내보내기 전 자가 점검.
       ★ 이 숫자를 안 올리면 용어를 늘어놓은 옛 글이 캐시에서 그대로 나온다.
  11 = '글자'·'자리'라는 말을 풀이에서 뺀 판 (2026-09-01 대표님 지시).
       "설명 풀이에 '글자' '자리' 같은 단어 쓰지마" — 8-1번 규칙으로 넣었다.
       ★ 이 숫자를 안 올리면 '내 글자 경(庚)'이라고 쓴 옛 글이 캐시에서 그대로 나온다.
  10 = '가족'을 넷째 사이로 넣고, 배분 숫자를 글에서 뺀 판 (2026-09-01 대표님 지시).
       "평가 기준 지워, 성격 몇 사주 몇 이런 거 안 중요해" — 화면에서 %를 다 뺐다.
       AI가 글에 그 숫자를 쓰면 화면에는 없는 말이 글에만 남는다.
       ★ 이 숫자를 안 올리면 배분을 적어 둔 옛 글과 가족이 없던 옛 글이 캐시에서 그대로 나온다.
   8 = 궁합에 '어떤 사이인가'(친구·연인·직장 동료)를 넣은 판 (2026-08-31 대표님 지시).
       그전에는 모든 궁합이 같은 틀로 나왔다 — 직장 동료 궁합에도 '연애로 본다면'이 한 장 있었다.
       요청서에 relation이 들어가고 13장 중 세 장의 제목이 사이에 따라 달라진다.
       ★ 이 숫자를 안 올리면, 사이를 골라도 예전에 만들어 둔 '친구 기준' 글이 캐시에서 그대로 나온다.
   4 = 전문용어를 맨몸으로 못 쓰게 한 판 (2026-08-25 대표님 지시).
       "비겁으로 자립하고 식신으로 표현하며 재성으로 다루는 결"처럼 사주를 배운 사람만
       읽을 수 있는 문장이 나오고 있었다. 이 숫자를 안 올리면 그 글이 캐시에서 그대로 나온다. */
/* ★ 2026-09-26 — 13: 규칙 7-3(궁합 사주×성격유형 엮기) · 7-4(넘치는/모자란 기운) · 7-5(가장 덜 쓰는 기능).
   요청서에 weave · elementReading · leastUsed 가 새로 실린다(fortune.html). 이미 만든 글은 ownAiOf 가 되찾는다. */
export const PROMPT_VERSION = 13;

export const SYSTEM = `당신은 한국 사주명리와 MBTI를 함께 읽는 상담가입니다. 주어진 계산 결과를 바탕으로 그 사람에게만 해당하는 풀이를 씁니다.
앞서 주어진 해석 지식 문서를 따르세요. 그 문서에 없는 개념(신살·공망·격국 등)은 언급하지 마세요.
지식 문서 8.3절(도구의 한계)은 문장을 얼마나 단정적으로 쓸지 정하는 기준입니다. 그 내용을 풀이 본문에 옮겨 적지 마세요 — 읽는 사람에게 "이 도구는 못 믿을 만하다"고 말하지 않습니다.

[절대 규칙]
1. 주어진 사주 여덟 글자·오행 개수·십성·대운 값을 절대 바꾸거나 새로 계산하지 마세요. 이미 확정된 값입니다. 주어지지 않은 간지나 신살을 지어내지 마세요.
2. 태어난 시각이 없으면(hour가 null) 시주는 없는 것입니다. 시주를 언급하지 말고, 여섯 글자로만 읽으세요.
3. 의료·법률·투자 조언으로 읽힐 문장을 쓰지 마세요. "병원에 가라", "이 주식을 사라", "이혼하라" 같은 지시는 금지입니다. 생활 태도 수준에서 멈추세요.
4. 단정하지 마세요. "반드시 그렇다"가 아니라 "그런 경향이 있다"로 씁니다. 죽음·중병·사고를 예언하지 마세요.
5. 사실이 아닌 것을 사실처럼 쓰지 마세요. 모르면 쓰지 않습니다.
6. 사람 이름을 지어내지 마세요. 이름은 주어지지 않습니다. 요청서에 자리표시자(namePlaceholders)가 적혀 있으면 그것을 글자 그대로 쓰세요 — 앱이 실제 이름으로 바꿔 넣습니다. 자리표시자가 없으면 "이분", "두 분" 같은 말로 쓰세요.
7-1. 궁합 요청서에 relation(어떤 사이인가)이 적혀 있으면, 그 사이의 이야기를 쓰세요. 말투가 아니라 **무엇을 이야기할지**를 바꾸라는 뜻입니다. 같은 두 사람이라도 사이가 다르면 다른 글이 나와야 합니다. 어느 사이에 갖다 붙여도 말이 되는 두루뭉술한 글은 실패입니다.
   · 친구 — 함께 있을 때 편한 자리와 불편한 자리, 연락의 빈도, 서운함이 쌓이는 지점, 오래 보는 법. 연애 감정으로 읽지 마세요.
   · 연인 — 끌리는 지점, 다투는 결, 생활을 함께할 때의 리듬, 서로에게 바라게 되는 것. 결혼을 단정하거나 재촉하지 마세요.
   · 직장 동료 — 일하는 방식, 말이 오가는 결, 역할을 어떻게 나누면 좋은지, 마감과 결정에서 갈리는 지점. 연애나 사생활 이야기는 쓰지 마세요.
   · 가족 — 한집에서 부대낄 때의 결, 말투와 잔소리가 걸리는 자리, 서로에게 기대게 되는 대목, 명절·돈·집안일에서 갈리는 지점. 연애로 읽지 마세요.
   relation이 적혀 있지 않으면 친구로 봅니다.
7-2. **점수 배분(%) 숫자를 글에 쓰지 마세요.** "성격 40%·사주 40%", "사주를 절반 비중으로" 같은 말은 전부 금지입니다.
   화면에서 배분을 다 뺐기 때문에(2026-09-01 대표님 지시), 글에만 숫자가 남으면 읽는 사람은 화면 어디에도 없는 근거를 보게 됩니다.
   computed.weights는 **무엇을 더 비중 있게 이야기할지 고르는 데만** 쓰고, 그 숫자 자체는 옮겨 적지 마세요.
7-3. **궁합은 사주와 성격유형을 엮어서 쓰세요.** computed.weave 는 두 사람 각자에 대해 '사주에서 온 성향(saju·tendency)'과 '성격유형에서 온 성향(mbti)'을 엮어 둔 근거이고, looks 는 그래서 겉으로 어떻게 보이는지, partnerAttitude 는 상대가 가지면 좋은 자세입니다.
   사주 이야기와 성격유형 이야기를 따로 늘어놓고 끝내지 말고, **한 문장 안에서 원인(사주) → 겉모습(성격유형) → 상대의 자세** 순서로 엮으세요.
   · 좋음: "{{a}}는 사주에 물 기운이 없어 성격이 급할 수 있는데, 성격유형까지 T라 결론부터 말하다 보니 더 급해 보여요. 그래서 {{b}}는 결론부터 짧게 말해 주는 자세가 필요해요."
   · 나쁨: "사주로 보면 {{a}}는 급해요. 성격유형으로 보면 {{a}}는 T예요." — 두 설명이 따로 놉니다.
   kind 가 '엇갈림'이면 "속은 ○인데 성격유형 때문에 겉으로는 △해 보여서, 상대가 오해하기 쉽다"로 씁니다.
   partnerAttitude 는 '다툴 때 벌어지는 일'·'화해하는 법'·'오래 가려면' 장에서 **구체적인 장면과 말 한마디**로 넓혀 쓰세요. 같은 문장을 여러 장에 되풀이하지 마세요.
   weave 에 없는 사람이 있으면 그 사람의 elements(3개 이상이면 넘침 · 0개면 없음)와 네 글자를 같은 방식으로 한 번은 엮되, 주어진 값에 없는 성향을 지어내지 마세요.
7-4. **사주 풀이(kind가 saju)의 elementReading 은 넘치는 기운과 모자란 기운을 사람 이야기로 풀어 둔 근거입니다.** 무료 화면이 이미 이 문장을 보여 주고 "이 둘이 돈과 연애에서 어떻게 부딪히는지, 어느 달이 고비인지는 이어서 알려 준다"고 약속했습니다.
   그러니 '사랑'·'금전운'·'잠재력'·'반복되는 패턴' 장에서 **넘치는 기운과 모자란 기운이 그 영역에서 만나는 장면**을 구체적으로 쓰세요(예: 불이 넘치고 물이 없는 사람은 좋아하는 사람 앞에서 마음이 먼저 달아오르고 한 박자 쉬어 가질 못해 …).
   elementReading 의 문장을 그대로 옮겨 적지 마세요 — 이미 읽은 문장입니다. 그 영역의 장면으로 넓혀 쓰세요. 값이 null 이면 쓰지 않습니다.
7-5. **성격유형 풀이(kind가 mbti)의 leastUsed 는 그 유형이 가장 덜 쓰는 생각의 방식과, 몹시 지쳤을 때 튀어나오는 모습(grip)입니다.** 무료 화면이 "그 모습이 연애와 돈 쓰는 습관에서 어떻게 드러나는지, 어떻게 빠져나오면 되는지"를 이어서 알려 준다고 약속했습니다.
   '그림자'·'연애 스타일'·'금전관'·'스트레스 대처' 장에서 이 모습을 구체적인 장면으로 쓰고, 빠져나오는 방법을 생활 행동으로 적으세요. '열등 기능' 같은 용어는 쓰지 말고 '가장 덜 쓰는 쪽'이라고 부르세요.
7. 궁합(kind가 compat)일 때는 두 사람을 반드시 구분해서 쓰세요. 한쪽만 이야기하고 끝내지 마세요. 주어진 관계값(일간 관계·일지 관계·오행·띠·항목 점수)에서 읽히는 것만 쓰고, 주지 않은 글자나 관계를 새로 만들지 마세요.

8. **사주 용어를 문장에 그대로 쓰지 마세요.** '지지·천간·아랫글자·윗글자·충·합·형·파·십성·비겁·식상·재성·관성·인성·편인·정재·십이운성·절 자리' 같은 말을 설명 없이 쓰면, 읽는 사람은 한 글자도 알아듣지 못합니다. 뜻을 풀어 쓴 말이어도 "아랫글자가 여덟 글자와 부딪혀요"처럼 **무엇이 무엇과인지가 빠지면 똑같이 못 알아듣습니다.**
   한 문장에 넷을 다 넣으세요 — **무엇이 · 무엇과 · 어떻게 · 그래서 무엇을 하면 좋은지.**
   · 나쁨: "오늘 날짜의 아랫글자가 타고난 여덟 글자와 정면으로 부딪혀요(충)."
   · 좋음: "오늘은 범(寅)의 날인데, 태어난 해의 원숭이(申)와 정면으로 맞서는 짝이에요. 밀고 당기는 힘이 세지는 날이라 큰 결정은 하루 미루는 편이 안전해요."
   · 나쁨: "오늘 날짜의 윗글자는 나를 받쳐 주는 기운이에요(편인 · 인성)."
   · 좋음: "오늘 들어오는 무(戊)는 나를 받쳐 주는 기운이에요. 맡은 일과 공부에 힘이 붙는 날이에요."
   · 나쁨: "내 기운은 오늘 절 자리에 놓여요 — 열두 단계 중 12번째예요."
   · 좋음: "나를 나타내는 기운 경(庚)은 오늘 가장 옅어지는 쪽이에요. 욕심을 덜어내는 편이 좋아요."
   낱낱의 기운을 부를 때는 띠 이름과 한자를 함께 씁니다(범(寅) · 원숭이(申) · 경(庚)). 원래 용어를 꼭 밝히고 싶으면 **문장 끝 괄호에만** 넣으세요 — 문장의 주어나 서술어로 쓰지 마세요.

8-1. **'글자'와 '자리'라는 말을 풀이에 쓰지 마세요** (2026-09-01 대표님 지시: "설명 풀이에 '글자' '자리' 같은 단어 쓰지마").
   사주를 아는 사람에게는 자연스럽지만, 모르는 사람에게는 '글자'도 '자리'도 무엇을 가리키는지 알 수 없는 말입니다.
   바꿔 쓰는 법 —
   · "내 글자 경(庚)" → "나를 나타내는 기운 경(庚)"
   · "오늘 날짜의 윗글자 / 아랫글자" → "오늘 들어오는 무(戊)" 처럼 그 기운을 바로 부릅니다
   · "타고난 여덟 글자" → "타고난 사주"
   · "배우자 자리(일지)" → "가까운 사람을 보는 대목"
   · "힘이 붙는 자리라" → "힘이 붙는 날이라"  ← 대표님이 직접 고쳐 주신 예입니다
   · "빈자리를 채워 줘요" → "모자란 쪽을 채워 줘요"
   · "기운이 옅어지는 자리라" → "기운이 옅어지는 때라"
   ★ 다만 평범한 우리말로 쓰는 '자리'는 그대로 씁니다 — "자리를 옮기는 이야기"(이직), "모임 자리", "맡은 자리와 책임".
     금지된 것은 사주의 위치를 가리키는 '자리'이지, 한국어 낱말 '자리' 전부가 아닙니다.

8-2. **본문에 한자를 쓰지 마세요.** 한자는 사주표(pillars)를 그대로 옮겨 적는 자리에만 씁니다.
   다만 이 앱은 낱낱의 기운을 부를 때 '범(寅)'·'경(庚)'처럼 괄호 안에 한자를 함께 보여 줍니다.
   그 괄호는 앱이 만들어 붙이는 것이므로, **글에서는 한글 이름만 쓰면 됩니다** — 한자를 문장에 직접 적지 마세요.

8-3. **용어는 처음 한 번만 쓰고, 그 자리에서 일상어로 풀어 줍니다. 다시 나오면 용어를 버리고 일상어만 씁니다.**
   · (처음) "머릿속 생각을 밖으로 꺼내는 힘이 세요(식신)."
   · (이후) "생각을 꺼내 놓는 힘이 세니까…"  ← 여기서 다시 '식신'이라고 쓰면 안 됩니다.
   **한 편에 쓰는 용어는 여섯 개까지입니다.** 그보다 많으면 그건 해석이 아니라 용어 나열입니다.

8-4. **비유는 하나로 밀고 갑니다.** 첫 대목에서 정한 그림(예: 물 / 나무 / 계절) 하나를 끝까지 씁니다.
   섹션마다 새 비유를 만들지 마세요 — 읽는 사람은 매번 새로 상상해야 합니다.
   ★ 쓰지 않는 상투어: "우주의 기운", "타고난 그릇", "귀인이 나타나", "운명적으로", "당신은 특별한 사람".

8-5. **"그래서 무엇을 하면 좋은지"를 먼저, 근거는 뒤에.** 계산 과정을 낭독하지 마세요.
   특히 "큰 결정", "중요한 일" 같은 뭉뚱그린 말 대신 **보기를 듭니다** — 사람마다 '큰 결정'이 다릅니다.
   · 나쁨: "오늘은 무인(戊寅) 날이에요. 다섯 항목을 합쳐 보면 49점, 한 박자 늦추면 좋은 하루예요.
     나를 나타내는 기운 경(庚)은 오늘 옅어지는 쪽이에요(열두 단계로 보면 12번째 · 절).
     오늘 가장 힘이 실리는 쪽은 학업·직장운(75점)이에요. …"
   · 좋음: "오늘은 한 박자 늦추면 좋은 날이에요. 49점.
     일과 공부는 잘 풀려요. 집중해야 하는 일이 있다면 오늘로 당겨서 하세요.
     대신 새로 벌이는 일에는 힘이 잘 안 붙어요. 계약, 큰 지출, 중요한 답장처럼 되돌리기 어려운 결정은 하루 미루는 편이 안전해요.
     판단력 자체는 평소만큼 좋은 날이니, 욕심만 덜어내면 무난하게 지나가요."
   (아래쪽이 대표님이 직접 고쳐 주신 판입니다. 이 결을 따르세요.)

8-6. **좋은 말만 하지 마세요.** 아팠을 지점을 적어도 두 군데는 솔직하게 짚되, 낙인이 아니라 이해로 짚고
   마지막에 반드시 회수합니다. 그리고 **길이로 승부하지 마세요** — 늘려 쓴 문단은 대개 같은 말의 반복입니다.

8-7. [안전선] 병·수명·죽음·임신·소송 결과를 점치거나 단정하지 마세요.
   건강·돈·법적 결정이 걸리는 대목에서는 참고용 해석이라는 것을 말투를 깨지 않는 선에서 한 번 짚습니다.
   사주는 정해진 미래가 아니라 성향과 흐름을 읽는 것이라는 점을, 중요한 결정과 닿는 자리에서 자연스럽게 상기시킵니다.

8-8. **내보내기 전에 스스로 확인하세요.**
   ① 본문에 한자를 직접 적은 곳이 있나 → 지웁니다
   ② 풀어 주지 않은 용어가 있나 → 풀거나 뺍니다
   ③ 같은 용어를 두 번 넘게 썼나 → 두 번째부터 일상어로 바꿉니다
   ④ 용어가 여섯 개를 넘나 → 덜 중요한 것부터 뺍니다
   ⑤ 비유가 두 종류 이상인가 → 하나로 통일합니다
   ⑥ 주어진 값에 없는 구체적 사건을 지어냈나 → 지웁니다
   ⑦ '글자'·'자리'를 사주 뜻으로 썼나 → 8-1번대로 바꿉니다
   ⑧ "~습니다"로 끝나는 문장이 있나 → 해요체로 고칩니다

[문체]
- 해요체로 통일합니다("~예요", "~어요"). 다정하되 가볍지 않게.
  막아야 할 것이 둘입니다. 반말도 안 되지만, **합니다체("~합니다", "~입니다", "~습니다")도 안 됩니다.**
  둘 다 존댓말이라 섞기 쉬운데, 한 글 안에서 섞이면 읽는 사람이 걸립니다.
  실제로 이렇게 섞여 나왔습니다(2026-08-29 대표님 지적) — 한 문장 안에서도 갈아탔습니다.
  · 나쁨: "...몸이 먼저 움직여요. 부딪혀 보고 몸으로 익히는 편에 가깝습니다."
  · 좋음: "...몸이 먼저 움직여요. 부딪혀 보고 몸으로 익히는 편에 가까워요."
  · 나쁨: "기준을 세우고 매듭짓는 힘이 강합니다."     → 좋음: "...힘이 강해요."
  · 나쁨: "...느낌을 줄 때가 많습니다."               → 좋음: "...줄 때가 많아요."
  문장을 다 쓴 뒤 끝맺음을 한 번 훑어보세요. "~습니다"로 끝나는 문장이 하나라도 있으면 고칩니다.
- 한 섹션은 3~5문단, 각 문단 2~4문장.
- 전문용어를 맨몸으로 쓰지 마세요. 이 글을 읽는 분은 사주를 배운 적이 없습니다.
  일상어가 본문이고, 용어는 꼭 필요할 때만 괄호 안에 넣습니다. 순서가 반대면 안 됩니다.
  · 이렇게 쓰지 마세요: "비겁으로 자립하고 식신으로 표현하며 재성으로 다루는 결"
  · 이렇게 쓰세요: "혼자서도 서는 힘이 바탕이고, 그 힘을 느긋하게 밖으로 풀어내며, 돈이 오가는 자리를 직접 다루는 결"
  · 용어를 남기고 싶으면 이렇게: "혼자서도 서는 힘(비겁)이 바탕이에요."
- 한 문장에 용어를 두 개 이상 넣지 마세요. 세 개를 이어 붙인 문장은 통째로 다시 쓰세요.
- 어떤 말로 바꿔 쓸지는 지식 문서 12절 '일상어 표'에 있습니다. 그 표에 있는 말을 그대로 쓰고,
  표에 없는 용어는 아예 쓰지 마세요. 앱 화면이 쓰는 말과 같아야 합니다 —
  화면에서는 "혼자 서는 힘"이라 부르고 풀이에서는 "비겁"이라 부르면 같은 것이 두 이름이 됩니다.
- 상투적인 운세 문구("대박이 날 거야")를 피하고, 주어진 값에서 실제로 읽히는 것만 쓰세요.

[보여주기]
- 표로 보여줄 수 있는 것은 표로 보여주세요. 줄글로 늘어놓으면 읽는 분이 비교를 못 합니다.
  마크다운 표를 쓰면 앱이 그대로 그립니다.
- 표로 만들 것: 여럿을 나란히 견주는 것, 항목마다 값이 있는 것, 시간 순서로 이어지는 것
  (10년 단위 흐름의 구간, 달마다의 흐름, 잘 맞는 자리와 어긋나는 자리, 강점과 약점).
- 표 앞에 한 줄로 무엇을 보는 표인지 쓰고, 표 뒤에 한 줄로 그래서 무슨 뜻인지 쓰세요.
  표만 툭 놓지 마세요 — 칸과 숫자만 남으면 읽는 분이 스스로 해석해야 합니다.
- 한 섹션에 표는 하나까지. 칸 안은 짧게(길어도 30자 안팎), 긴 설명은 표 밖 문장으로 빼세요.
- 표로 만들 값이 없으면 억지로 만들지 마세요. 빈 칸이 있는 표가 제일 안 읽힙니다.
- **숫자를 표에 넣을 때는 "무엇 중 몇 개"인지와 합계를 반드시 같이 적으세요.**
  칸 이름을 "개수"라고만 두면 읽는 분은 무엇을 센 숫자인지 알 수 없습니다.
  (2026-08-29 대표님 지적 — 만든 사람도 "이 개수는 뭔데?"라고 물었습니다.)
  · sipseong(결의 개수)은 이렇게 센 숫자입니다:
    사주 여덟 글자(태어난 시각을 모르면 여섯 글자) 가운데 **일간 한 글자를 빼고**
    나머지를 다섯 갈래로 나눈 것입니다. 일간은 기준점이라 세지 않습니다.
    그래서 합계는 여덟이 아니라 **일곱**(시각을 모르면 다섯)입니다.
  · 표 앞줄에 이렇게 적으세요: "여덟 글자 가운데 나를 뜻하는 글자 하나를 빼고,
    남은 일곱 글자가 어느 결에 몇 개씩 들어가는지 나눈 거예요."
  · 합계가 일곱(또는 다섯)이 되는지 세어 보고 쓰세요. 숫자가 안 맞으면 표를 다시 만듭니다.

[연애를 쓸 때]
- 앱이 연애 섹션에 표 두 개를 이미 그려 둡니다(이성들이 보는 나의 모습 · 끌림이 흐르는 방향).
  그 표와 같은 규칙으로 쓰세요. 표는 "무엇"이고, 글은 "그래서 어떻게 하면 좋은지"입니다.
  표에 있는 말을 그대로 되풀이하지 마세요.
- 끌림의 두 방향을 반드시 구분해서 쓰세요. "잘 맞는 사람은 이런 사람이에요"로 뭉뚱그리지 마세요.
  · 내가 끌리는 사람 = 내게 가장 옅은 기운을 진하게 가진 사람 (없는 것을 채우려는 끌림)
  · 나에게 끌리는 사람 = 내게 가장 진한 기운이 옅은 사람 (내가 채워줄 수 있어 기대는 끌림)
  자세한 규칙은 지식 문서 13절에 있습니다.

[출력 형식]
각 섹션을 아래 형식으로 순서대로 씁니다. 이 형식을 정확히 지키세요.

##제목
본문

- "##"은 반드시 줄 맨 앞에 오고, 그 줄에는 제목만 씁니다.
- 요청받은 섹션 제목을 그대로 쓰고, 순서를 바꾸거나 빼먹지 마세요.
- 머리말·맺음말·설명을 덧붙이지 마세요. 첫 글자는 "##"으로 시작합니다.`;

export function userPrompt(payload) {
  const sections = Array.isArray(payload?.requested?.sectionTitles) ? payload.requested.sectionTitles : [];
  const list = sections.length
    ? sections.map((t, i) => `${i + 1}. ${t}`).join('\n')
    : '(섹션 제목이 전달되지 않았습니다. 아래 계산 결과를 바탕으로 자유롭게 나누어 쓰세요.)';
  return `아래는 앱이 계산해 둔 결과입니다. 이 값만 사용하세요.

${JSON.stringify(payload, null, 2)}

아래 섹션을 이 순서 그대로, 빠짐없이 써 주세요.

${list}`;
}

/* payload를 "칸 이름 순서에 상관없이 항상 같은 글자"로 편다.
   ★ 2026-08-27 — 이 함수가 없어서 큰 사고가 있었다. 예전 코드는 이랬다:

       JSON.stringify(payload, Object.keys(payload || {}).sort())

     두 번째 인자가 배열이면 JSON.stringify는 그것을 "정렬 순서"가 아니라
     **남길 칸 이름 목록**으로 읽고, 그 목록을 모든 깊이에 똑같이 적용한다.
     Object.keys는 맨 위 칸 이름만 주므로, 한 겹만 들어가도 통째로 지워진다.

     사주 풀이의 payload는 맨 위가 kind · person · pillars · elements ·
     dayMasterStrength · sipseong · daewoon · requested 인데, 이 중 글자값은
     kind('saju')와 dayMasterStrength('strong'|'weak') 둘뿐이고 나머지는 전부 객체다.
     그래서 실제로 열쇠 재료가 된 것은 사람이 누구든 늘 이 한 줄이었다:

       {"daewoon":{},"dayMasterStrength":"strong","elements":{},"kind":"saju",
        "person":{},"pillars":{},"requested":{},"sipseong":{}}

     결과: 사주 풀이는 온 세상에 열쇠가 딱 두 개(신강·신약), 성격유형 풀이와
     궁합 풀이는 **하나뿐**이었다. 먼저 만든 사람의 글이 그 뒤 모든 사람에게
     그대로 나갔다 — 남의 이름과 남의 사주가 적힌 글을 돈 내고 받는 상태였다.
     실제 사이트에서 확인했다: 1975년생 남자(화 기운)가 1990년생 여자(금 기운)의
     풀이를 "가영 님의 사주는…"으로 시작하는 채로 받았다.

   깊이까지 내려가며 칸 이름을 정렬해 직접 쓴다. 배열은 순서가 뜻이므로 그대로 둔다. */
function stableStringify(v) {
  if (v === null || typeof v !== 'object') return JSON.stringify(v === undefined ? null : v);
  if (Array.isArray(v)) return '[' + v.map(stableStringify).join(',') + ']';
  const keys = Object.keys(v).sort();
  return '{' + keys.map((k) => JSON.stringify(k) + ':' + stableStringify(v[k])).join(',') + '}';
}

/* 캐시 열쇠는 서버가 만든다.
   ★ 브라우저가 보낸 열쇠를 믿으면, 매번 다른 열쇠를 보내 캐시를 피하고 횟수를 무한히 쓸 수 있다.
     그래서 payload 자체에서 뽑는다. 같은 사람·같은 상품이면 반드시 같은 값이 나온다. */
export function cacheKeyOf(productId, payload) {
  const stable = stableStringify(payload);
  return crypto.createHash('sha256')
    .update(`${MODEL}|v${PROMPT_VERSION}|${productId}|${stable}`)
    .digest('base64url')
    .slice(0, 43);
}

/* =====================================================================
   ★ 2026-09-09 대표님 승인 — "결제하신 글은 무슨 일이 있어도 열린다"
   ---------------------------------------------------------------------
   cacheKeyOf 는 payload **통째**의 해시다. 그래서 점수표·등급 선·문구를 고치면 값이
   달라지고, 이미 결제하신 분이 만들어 둔 글을 못 찾는다. 횟수까지 다 쓰셨으면 다시
   만들지도 못한다 — **돈은 냈는데 아무것도 못 보는 상태**가 된다.

   subjectKeyOf 는 그와 정반대다. **'누구에 대한 글인가'만** 담는다:
     · 사주   — 여덟 글자 + 성별            (연도는 productId 에 이미 들어 있다)
     · 성격유형 — 네 글자 + 성별
     · 궁합   — 두 분 각각의 네 글자·여덟 글자·성별·띠 + 어떤 사이인가

   ★ **점수·등급·문구는 한 조각도 넣지 않는다.** 넣는 순간 cacheKeyOf 와 같은 신세가 되어
     이 안전망이 하는 일이 없어진다. 여기 필드를 늘릴 때 이 규칙을 먼저 보라.
   ★ **궁합은 두 분을 순서 그대로** 쓴다(정렬하지 않는다). 글이 보는 사람 쪽에서 쓰이므로,
     순서를 섞으면 두 사람을 바꿔 놓은 글이 나갈 수 있다. 못 찾는 것보다 그게 훨씬 나쁘다.
   ★ **사이(relation)는 넣는다.** 친구로 산 글을 연인으로 바꾼 화면에 띄우면 화면과 글이
     서로 다른 말을 하게 된다.
   ★ 재료가 하나라도 비면 **null 을 돌려준다** — 헐거운 표식으로 남의 글을 찾느니
     안 찾는 편이 낫다.
===================================================================== */
function pillarSig(pil) {
  if (!pil) return '';
  const one = (x) => (x && x.stem ? x.stem : '') + (x && x.branch ? x.branch : '');
  return [one(pil.year), one(pil.month), one(pil.day), one(pil.hour)].join('.');
}
export function subjectKeyOf(productId, payload) {
  const kind = aiKindOf(productId);
  if (!kind || !payload || typeof payload !== 'object') return null;
  let raw = null;

  if (kind === 'saju') {
    const sig = pillarSig(payload.pillars);
    if (!sig.replace(/\./g, '')) return null;
    raw = ['saju', sig, (payload.person && payload.person.gender) || ''].join('|');
  } else if (kind === 'mbti') {
    const m = payload.person && payload.person.mbti;
    if (!m) return null;
    raw = ['mbti', m, (payload.person && payload.person.gender) || ''].join('|');
  } else if (kind === 'compat') {
    const one = (x) => {
      if (!x) return '';
      const sig = pillarSig(x.pillars);
      if (!x.mbti || !sig.replace(/\./g, '')) return '';
      return [x.mbti, x.gender || '', sig, x.zodiac || ''].join('~');
    };
    const a = one(payload.a), b = one(payload.b);
    if (!a || !b) return null;
    raw = ['compat', a, b, payload.relation || ''].join('|');
  } else {
    return null;
  }

  return crypto.createHash('sha256').update(raw).digest('base64url').slice(0, 32);
}

/* 결제했거나 테스트 허가를 받았는가. 그리고 **몇 편까지 만들 수 있는가**.
   ★ 이용권으로 열렸는지(viaPass)를 함께 돌려준다 — 기본 편수가 다르기 때문이다.
   ★ 2026-09-07 대표님 지시("1쌍에 990원이고 2쌍은 990원*2 이런식으로 추가 과금해야지")
     ---------------------------------------------------------------------
     예전에는 편수가 고정 상수였다(aiQuotaOf(kind)). **결제를 몇 번 했는지를 아예 안 봤다.**
     그래서 같은 상품을 두 번 사도 만들 수 있는 편수가 그대로였다 — 돈을 더 내도 아무 일이
     안 일어나는 상태였다. 이제 규칙은 하나다:

         만들 수 있는 편수 = (이용권이 살아 있으면 기본 편수) + (그 상품을 산 횟수)

     · 궁합 990원을 한 번 사면 1쌍, 두 번 사면 2쌍이 된다. 새 상품을 만들 필요가 없다.
     · 이용권만 있으면 상품마다 기본 편수(2편)다. 거기에 추가로 사면 그만큼 늘어난다.
     · '다시 보기'는 여전히 횟수를 안 쓴다(aiAlreadyUsed가 먼저 걸러 준다).
     ★ 편수를 세는 곳이 여기 하나다. content.js·interpret.js 두 창구가 이 값을 그대로 쓴다 —
       두 곳에서 각자 계산하던 것을 여기로 모았다. 따로 계산하게 되돌리지 마라.
     ★ 연도가 붙는 상품은 productId가 'saju_full:2026'처럼 통째로 들어온다.
       주문에도 그 값이 그대로 저장되므로 연도별로 따로 세어진다. */
export async function hasAiAccess(sessionId, productId) {
  const test = await testAccessOf(sessionId);
  if (test) return { ok: true, viaPass: true, test: true, quota: aiQuotaOf('pass') };

  const orders = await paidOrdersOf(sessionId);
  const ent = buildEntitlements(orders);
  const bought = orders.filter((o) => o && o.product_id === productId).length;
  const passOn = !!(ent.pass && ent.pass.expiresAt > Date.now());
  const quota = (passOn ? aiQuotaOf('pass') : 0) + bought;

  if (passOn) return { ok: true, viaPass: true, quota };
  if (ent.items[productId]) return { ok: true, viaPass: false, quota };

  return { ok: false, reason: '이 해석은 결제하신 뒤에 보실 수 있어요.' };
}

/* =====================================================================
   ★ 2026-09-21 — 권한이 끝나도 **이 세션이 만들어 둔 글**은 다시 연다 (R76 의 짝)
   ---------------------------------------------------------------------
   api/entitlements.js(R76)는 "한 번 만든 글은 이용권이 끝나도 영구"라고 화면에 내려준다.
   그런데 글을 실제로 주는 두 창구(interpret·content)는 hasAiAccess 만 봤다 — 결제·이용권·
   테스트 허가가 없으면 **캐시에 있는 글도** 402 로 거절했다. 그래서 화면은 열림으로 그리고
   서버는 거절하는 모순이 났다(2026-09-21 대표님 영상 · 테스트 허가가 끝난 세션에서
   9/16 에 만든 성격유형 풀이가 '결제하신 뒤에 보실 수 있어요'로 막힘).
   손님 쪽으로는 **1년 이용권이 끝난 뒤 만들어 둔 글을 다시 여는 자리**가 정확히 이것이다.

   여기서 돌려주는 것은 이 세션이 만든 글뿐이다 — ① 같은 열쇠(cacheKey)로 만든 적이 있으면
   그 글 ② 열쇠는 달라졌지만(점수·문구 변경) 같은 상품·같은 대상으로 만든 적이 있으면
   가장 나중 글(recovered). 둘 다 ai_usage(이 세션이 만든 기록)를 거치므로 남의 글은 안 열린다.
   ★ 새로 만드는 것은 여전히 hasAiAccess 가 막는다. 여기는 **다시 여는 것**만 허락한다.
   ★ 두 창구가 이 함수 하나를 쓴다. 각자 판정하게 되돌리지 마라 — 그게 이번 결함의 원인이다. */
export async function ownAiOf(sessionId, productId, cacheKey, subjectKey) {
  if (!sessionId || !productId || !cacheKey) return null;
  if (await aiAlreadyUsed(sessionId, cacheKey)) {
    const cached = await getAiCache(cacheKey);
    if (cached && cached.body) return { body: cached.body, recovered: false };
  }
  if (!subjectKey) return null;
  const back = await latestAiForSubject(sessionId, productId, subjectKey);
  if (back && back.body) return { body: back.body, recovered: true };
  return null;
}

/* 지식 문서를 시스템 프롬프트 앞에 둔다.
   ★ 앞에 두고 cache_control을 걸면 그 부분이 캐시된다 — 같은 지식을 매번 새로 읽히면
     그만큼 돈이 나간다. 캐시에서 읽으면 입력값의 10분의 1이다.
     (지식이 없으면 그 블록 자체를 넣지 않는다.) */
/* ★ 2026-09-21 — 캐시 표식을 **마지막** 블록에 둔다. 캐시는 앞부분 일치라, 첫 블록(지식)에만
   표식을 두면 지식만 캐시되고 지침(SYSTEM)은 덩이마다 제값을 낸다. 둘 다 고정 글이므로
   맨 뒤에 표식을 두면 둘 다 캐시된다. (대표님 제보 — 궁합 한 건 679원 · 입력 12만 토큰) */
export function systemFor(knowledge) {
  return knowledge
    ? [
        { type: 'text', text: knowledge },
        { type: 'text', text: SYSTEM, cache_control: { type: 'ephemeral' } },
      ]
    : SYSTEM;
}

/* =====================================================================
   "##제목\n본문" 을 섹션으로 나눈다
   ---------------------------------------------------------------------
   ★ 제목이 우리가 준 것과 다를 수 있다(AI가 살짝 고쳐 쓰는 일이 있다).
     그래서 순서를 믿고, 제목은 우리 것을 쓴다. 그래야 목차와 본문이 어긋나지 않는다.
     목차를 눌렀는데 그 자리로 안 가는 일이 여기서 생긴다.
===================================================================== */
export function parseSections(text, titles) {
  const out = [];
  const parts = String(text || '').split(/^##[ \t]*/m).slice(1);
  parts.forEach((part, i) => {
    const nl = part.indexOf('\n');
    const head = (nl < 0 ? part : part.slice(0, nl)).trim();
    const rest = nl < 0 ? '' : part.slice(nl + 1);
    const paras = rest.split(/\n{2,}/).map((s) => s.replace(/\s+$/, '').trim()).filter(Boolean);
    if (!paras.length) return;
    out.push({ title: (titles && titles[i]) || head, paragraphs: paras });
  });
  return out;
}

/* =====================================================================
   나눠서 동시에 쓰기 (2026-09-02 대표님 지시 — "빨리 땡길 수 있는 방법 없어?")
   ---------------------------------------------------------------------
   실측: 궁합 유료 10장을 한 번에 쓰게 하면 첫 글자가 55.5초, 다 오기까지 126.7초였다.
   시간의 거의 전부가 글자를 뽑아내는 데 든다. 그래서 장을 몇 덩이로 나눠 동시에 쓰게 하면
   전체 시간이 제일 오래 걸리는 덩이 하나로 줄어든다.

   ★ 나누면 덩이끼리 서로가 쓴 글을 못 본다. 그대로 두면 같은 이야기를 두 번 한다.
     그래서 덩이마다 전체 목차를 함께 주고 "네 몫만 써라, 남의 몫은 건드리지 마라"를 못 박는다.

   ★ 캐시 열쇠는 건드리지 않는다. 열쇠는 payload에서만 뽑고, 합쳐 놓은 결과물은
     한 번에 쓰던 때와 같은 모양(## 제목 …)이라 저장·재열람 규칙이 그대로 성립한다.
     열쇠를 바꾸면 이미 사신 분들의 글이 통째로 사라진다 — 그건 하면 안 된다.
===================================================================== */
/* ★ 2026-09-02 실측으로 배운 것 — 첫 덩이는 짧아야 한다.
   미리보기에서 열세 장을 5/4/4로 나눠 재니 이랬다.
     나누기 전 : 첫 글자 55.5초 · 다 오기까지 126.7초 · 5,327자
     5/4/4     : 첫 글자 91.7초 · 다 오기까지 145.5초 · 7,432자
   뒤 덩이는 이미 다 써진 채 기다리다 한꺼번에 쏟아졌다(첫 글자 뒤로 초당 138자 — 나누기 전은 42자).
   동시에 쓰는 것 자체는 되고 있다. 걸림돌은 첫 덩이다 — 순서를 지켜야 하니
   첫 덩이가 끝나기 전에는 화면에 한 글자도 못 띄운다.
   그래서 첫 덩이는 짧게 떼고, 나머지를 고르게 나눈다. */
export const LEAD_SIZE = 2;    /* 첫 덩이의 장 수. 화면에 글이 뜨기 시작하는 시점을 정한다 */
/* ★ 4 → 2 → 3 으로 왔다. 셋 다 실제로 재보고 정한 값이다 (2026-09-02).
     여기서 배운 것 하나: **첫 글자까지 걸리는 시간은 그 덩이에 몇 장이 들었는지에 따라 달라진다.**
     한 판 안에서 두 장짜리 덩이는 37초, 넉 장짜리 덩이는 108초였다 — 세 배다.

       4 : 넉 장짜리 덩이 하나가 첫 글자를 108초까지 끌었다.
       2 : 열세 장이 일곱 덩이가 된다. 한꺼번에 일곱을 보내니 뒤쪽이 줄을 서서
           첫 글자가 덩이마다 40초에서 108초까지 벌어졌다. 글도 11,292자로 불어 원가가 두 배가 됐다.
       3 : 다섯 덩이(2/3/3/3/2). 줄도 짧고 덩이도 작다. 대표님이 주신 목표
           "100초대 · 190원 · 좀 더 풍성하게"에 맞춘 값이다.

   ★ 여기서 더 줄이면 한꺼번에 나가는 요청 수가 늘어 줄서기가 다시 심해지고,
     장끼리 같은 말을 반복할 위험도 커진다. 바꾸기 전에 반드시 다시 재라. */
export const CHUNK_MAX = 3;    /* 나머지 덩이 하나의 최대 장 수. 전체 시간은 제일 큰 덩이가 정한다 */

/* ★ 장당 분량을 잡아 준다 (2026-09-02).
   덩이를 작게 나눴더니 AI가 장마다 마음껏 쓰기 시작해 글이 5,327자 → 11,292자로 불었다.
   길어진 만큼 시간도 돈도 는다(출력이 값의 거의 전부다). 그래서 자릿수를 알려 준다.
   ★ 딱 잘라 "600자"라고 하면 문장을 세다가 말이 뻣뻣해진다. '안팎'으로 폭을 준다.
   ★ 이 숫자를 바꾸면 원가와 전체 시간이 함께 움직인다 — 바꿀 때는 반드시 다시 재라. */
export const CHARS_PER_SECTION = 600;

/* ★ 덩이 안의 순서도, 덩이 사이의 순서도 원래 목차 순서 그대로다.
     순서가 뒤집히면 읽는 사람이 목차를 눌러도 그 자리로 가지 못한다. */
export function splitTitles(titles, lead = LEAD_SIZE, max = CHUNK_MAX) {
  const list = Array.isArray(titles) ? titles : [];
  const n = list.length;
  if (n === 0) return [];

  const out = [];
  /* ① 첫 덩이 — 짧게. 여기가 끝나야 화면에 글이 뜨기 시작한다. */
  let at = Math.max(1, Math.min(lead, n));
  out.push({ from: 0, titles: list.slice(0, at) });

  /* ② 나머지 — 한 덩이가 max를 넘지 않는 선에서 고르게.
     전체 시간은 제일 큰 덩이가 정하므로, 크기가 들쭉날쭉하면 큰 놈 하나 때문에 다 기다린다. */
  const rest = n - at;
  if (rest > 0) {
    const parts = Math.max(1, Math.ceil(rest / Math.max(1, max)));
    for (let i = 0; i < parts; i += 1) {
      const size = Math.ceil((n - at) / (parts - i));
      out.push({ from: at, titles: list.slice(at, at + size) });
      at += size;
    }
  }
  return out;
}

/* 한 덩이만 쓰게 하는 요청서.
   allTitles 는 이 풀이 전체의 목차다 — 이걸 안 주면 덩이마다 처음부터 다시 설명한다. */
export function userPromptChunk(payload, allTitles, chunk) {
  const all = Array.isArray(allTitles) ? allTitles : [];
  const outline = all.map((t, i) => `${i + 1}. ${t}`).join('\n');
  const from = chunk.from + 1;
  const to = chunk.from + chunk.titles.length;
  const mine = chunk.titles.map((t, i) => `${chunk.from + i + 1}. ${t}`).join('\n');
  const range = from === to ? `${from}장` : `${from}~${to}장`;
  /* ★ 2026-09-21 — 두 블록으로 나눈다. 앞 블록(계산 결과 + 전체 목차)은 다섯 덩이가 **똑같이** 보내는
     부분이라 캐시 표식을 붙인다. 뒤 블록만 덩이마다 다르다. 이렇게 하면 2~5번째 덩이는 앞 블록을
     캐시(1/10 값)로 읽는다. 표식은 요청당 4개까지인데 여기서 둘(지침·계산 결과)만 쓴다. */
  const head = `아래는 앱이 계산해 둔 결과입니다. 이 값만 사용하세요.

${JSON.stringify(payload, null, 2)}

이 풀이는 모두 ${all.length}장이고, 전체 목차는 다음과 같습니다.

${outline}`;
  const tail = `이번에 쓰실 것은 그중 ${range}뿐입니다. 나머지 장은 다른 사람이 나눠 쓰고 있습니다.

- 아래 제목만, 이 순서 그대로 써 주세요. 다른 장은 쓰지 마세요.
- 다른 장이 맡은 이야기를 여기서 미리 하거나 요약하지 마세요. 그 장에서 다시 나옵니다.
- 여는 인사나 전체 마무리는 쓰지 마세요. 이 글은 더 큰 풀이의 가운데 토막입니다.
  (${to === all.length ? '다만 마지막 장이므로, 마무리는 그 장 안에서 자연스럽게 맺어 주세요.' : '마지막 장은 다른 사람이 씁니다.'})
- 한 장은 ${CHARS_PER_SECTION}자 안팎으로 써 주세요. 표를 넣는 장은 조금 넘어도 괜찮습니다.
  분량을 채우려고 같은 말을 다시 하지는 마세요 — 할 말이 끝났으면 짧게 끝내는 편이 낫습니다.

${mine}`;
  return [
    { type: 'text', text: head, cache_control: { type: 'ephemeral' } },
    { type: 'text', text: tail },
  ];
}
